// ─────────────────────────────────────────────────────────────────────────────
// components/service-card.tsx   [MODIFIED]
//
// CHANGES:
//   1. Props type changed from lib/services `Service` → lib/types `ServiceData`
//   2. isDown is now derived from service.status (DOWN|DEGRADED) — NOT a toggle
//   3. onToggle kept so "Simulate Anomaly" button still works locally for demo
//   4. ANOMALY / CRITICAL_SERIES imports removed — live data drives the card
//   5. Sparkline uses service.series directly (already normalised by API route)
//   6. StatusBadge now handles DEGRADED state separately
// ─────────────────────────────────────────────────────────────────────────────
...
import type { ServiceData } from "@/lib/types"   // CHANGED: was Service from lib/services
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col gap-3">
        ...
        {/* Infra tags — CHANGED: values from live ServiceData props */}
        <div className="flex flex-wrap gap-1">
          <InfraTag label="cpu" value={`${service.cpu}%`} critical={isDown} />
          <InfraTag label="mem" value={service.mem}        critical={isDown} />
          <InfraTag label="rps" value={service.rps}        critical={isDown} />
        </div>

        {/* Sparkline — CHANGED: uses service.series (live, normalised 0-1) */}
        <Sparkline data={service.series} isDown={isDown} />
        ...
        <div className="grid grid-cols-2 divide-x divide-white/[0.04] rounded-lg border border-white/[0.04] bg-white/[0.015]">
          <div className="flex flex-col gap-0.5 p-2.5">
            ...
            >
              {/* CHANGED: service.latency is live p99Ms from backend */}
              <AnimatedNumber value={service.latency} />
            ...
            <span className="text-[10px] uppercase tracking-wider text-zinc-600">P99 Latency</span>
          </div>
          <div className="flex flex-col gap-0.5 p-2.5">
            <span
              className={cn(
                "flex items-baseline gap-0.5 font-mono text-xl font-bold tracking-tighter tabular-nums",
                errorRateColor(service.errorRate, isDown),
              )}
            >
              {/* CHANGED: service.errorRate is live numeric % from backend */}
              <AnimatedNumber value={service.errorRate} decimals={2} />
              <span className="text-xs font-medium opacity-60">%</span>
            </span>
            <span className="text-[10px] uppercase tracking-wider text-zinc-600">Error Rate</span>
          </div>
        </div>

        {/* Action button — onToggle is optional; only for demo chaos simulation */}
        {onToggle && (
          <Button
            onClick={onToggle}
            variant="outline"
          ...
          >
            {isDown ? <RotateCw className="h-3.5 w-3.5" /> : <Zap className="h-3.5 w-3.5" />}
...
function InfraTag({ label, value, critical }: { label: string; value: string; critical: boolean }) {
  return (
    <span
      className={cn(
...
// CHANGED: handles UP | DOWN | DEGRADED states
function StatusBadge({ status }: { status: ServiceData["status"] }) {
  if (status === "DOWN") {
    return (
  ...
  if (status === "DEGRADED") {
    return (
      <Badge className="shrink-0 gap-1 border-amber-500/30 bg-amber-500/10 text-xs text-amber-300 hover:bg-amber-500/10">
        <span className="relative flex h-1.5 w-1.5">
...
function Sparkline({ data, isDown }: { data: number[]; isDown: boolean }) {
  ...
  const fillId = `spark-${isDown ? "d" : "u"}-${Math.random().toString(36).slice(2, 6)}`
